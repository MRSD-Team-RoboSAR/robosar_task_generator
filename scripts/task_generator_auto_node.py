...
def handle_taskgen_getwaypts(req):
    ...
    dims = 2
    waypts = waypoints_array #waypoints[:,0:2].flatten()
    
    # Clear previous waypoints
    evt.clear()
    num_waypoints = 0
    waypoints_array = []
    return taskgen_getwayptsResponse(waypts)
...
def task_generator_server():
    global pub
    rospy.init_node('auto_task_generator_server')
    s = rospy.Service('auto_taskgen_getwaypts', auto_taskgen_getwaypts, handle_taskgen_getwaypts)
    rospy.Subscriber("generated_points", PointStamped, callback)
